...
how the attributes will be displayed in the information popup of the `Identify Tool`.
...
The display type by default is *label* but for attributes that contain URL an hyperlink will be showed 
or if the attribute contains a path to media file, the media itself will be rendered.

It is possible to define a completely custom ``HTML`` template for the *Get Feature Info* outcome. 
That is possible by enabling the :guilabel:`Use a custom template?` flag as shown in the figure below.


  .. figure:: ../_static/metadata_attributes_custom.png
      :align: center
      :width: 70%
...
On the top right of the page the button :guilabel:`Advanced Metadata` will open a single page
...
:guilabel:`Update` on the bottom of the page. A confirmation message will appear.
